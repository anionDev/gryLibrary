...
namespace GRYLibrary.Miscellaneous.Playlists
{
    public abstract class AbstractPlaylistHandler
    {
        ...
        protected abstract IEnumerable<string> GetSongsFromPlaylist(string playlistFile);
        ...
        private IEnumerable<string> GetSongsFromPlaylist(string playlistFileName, bool removeDuplicatedItems, bool loadTransitively, ISet<string> excludedPlaylistFiles, string workingDirectory)
        {
            ...
            IEnumerable<string> referencedFiles = this.GetSongsFromPlaylist(Path.Combine(workingDirectory, playlistFileName)).Where(item => IsAllowedAsPlaylistItem(item));
